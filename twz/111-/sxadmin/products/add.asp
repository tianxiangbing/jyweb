...
'注意取产品内容的方法，因为对大表单的自动处理，一定要使用循环，否则大于100K的内容将取不到，单个表单项的限制为102399字节（100K左右）
	...
	sql="insert into products(ProName,EnProName,ClassId,Content,Encontent,Power,isSuggest,ProPic,ProSmallPic,d_savefilename,d_savepathfilename,languages) values('"&ProName&"','"&EnProName&"',"&proClass&",'"&scontent&"','"&Encontent&"',"&Power&","&isSuggest&",'"&ProPic&"','"&ProSmallPic&"','"&d_savefilename&"','"&d_savepathfilename&"','"&languages&"')"
	'response.Write(sql)
	'response.End()
	conn.execute(sql)
	ok "发布成功！","add.asp"
