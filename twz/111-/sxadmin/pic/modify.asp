...
<%
 ...
 if request.querystring("action")="edit" then' 取提交过来的数据
	...
	sql="update [pic] set title='"&title&"',Entitle='"&Entitle&"',ClassId="&newsClass&",content='"&scontent&"',Encontent='"&Encontent&"',Power="&Power&",editTime=now(),newsPic='"&newsPic&"',d_savefilename='"&SaveFileName&"',d_savepathfilename='"&SavePathFileName&"' where id="&id
	'response.write(sql)
	'response.End()
	conn.execute(sql)
