...
|级联菜单的实现
|
|级联菜单，就是有两个select选项，选中其中一个后，另外一个选项也跟着变化。
+----------------------------------------
...
/*
用来初始化省份的菜单,将省份添加到slect中去
*/
function init(initProvince,initCity)
{
	...
	for(var i=0,n=p.length;i<n;i++)
	{
		...
		{
			opt.selected=true;	
			change_province(i,initCity)
			//alert(document.getElementById("hobby").options[i].index);
		}
		//有些浏览器不支持options属性的add方法，
		//但支持DOM的appendChild方法（比如：Konqueror）
		if(so.options.add)
		{
			so.options.add(opt);
		}
...
/*
*改变省份,将对应的城市添加到第二个select中去
*/
function change_province(pid,sid)
{
	var so=document.getElementById('city');
	so.length=0;
	//pid=c[pid].value.index
