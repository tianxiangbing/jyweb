    ...
    <table width="778" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="778" border="0" cellpadding="0" cellspacing="0" class="table_bottom2">
          <tr>
           <td width="123" height="35" class="t_left">新闻中心</td>
            <td align="right" class="t_right">您所在的位置：首页  &gt; 新闻中心<% if classid<>0 then  %>  &gt; <% =trim(RsC("classname")) %> <% end if %></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<%
 if classid <> 0 then
	sql = "select id,title,ClassId,addtime,power from news where ClassId="&classid&" order by power desc,classid asc"
	else
	sql="select id,title,ClassId,addtime,power from news order by power desc,classid asc"		
end if
'response.Write(sql)
...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">					 
 ...
 <td width="21" style="border-bottom:dotted 1px #ccc"><span class="tr_bot"><img src="images/news_icon.png" alt="" /></span></td>				
    <td width="610" align="left" style="border-bottom:dotted 1px #ccc; padding-left:9px;" valign="middle">
      <a href="news_show.asp?NewsId=<% =rs("id") %>" title="<% =trim(rs("title")) %>"><font style="color:;"><% =trim(rs("title")) %></font></a>
   </td>
...
<tr><td height="30" align="center"><% if rs.RecordCount>0 then %>  
  <% if i>0 then showpage("news.asp?classid="&classid&"&") %>
  <%
rs.close
set rs=nothing
%> 
