...
$(function(){
    ...
    function imgLocation(){
        ...
        if(Sys.ie){   //IE
            margin=4;
            box_width=$(".wedding_list").innerWidth();
        }
...
//if(Sys.opera) document.write('Opera: '+Sys.opera);
        if(Sys.safari){
            margin=5;
            box_width=$(".wedding_list").width();
        }


        var li = $(".wedding_list li");
        var li_width = $(".wedding_list li:eq(0)").width() + 20;
        var arr = []; //记录区域高度的数组
        ...
        for(var i = 0,len = li.length; i<len; i++){
            var li_hei = $(".wedding_list li").eq(i).outerHeight() + 13;
            if(i < num){
                arr[i] = li_hei;//把每个li放进数组
                li.eq(i).css({"top":0,"left":i * li_width});//第一行li的top值为0,左坐标是i*li_width;
            }else{
                min_hei = Math.min.apply(null,arr);
                minKey = getArraykey(arr,min_hei);
                arr[minKey] += li_hei;
                li.eq(i).css({"top":min_hei,"left":minKey * li_width});
            }
            $(".wedding_list").height(Math.max.apply(null,arr));
        }
    }
